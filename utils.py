...
def generate_gemini_insights(api_key, model_name, data, custom_prompt):
    ...
    prompt = f"""Anda adalah AI Copywriter dan Analis Data profesional. Anda bertugas menyusun teks pengantar (cover letter) dan poin-poin analisis (insights) untuk Laporan Penggunaan Layanan AI Chatbot SYGMA PT Adira Dinamika Multi Finance Tbk.
Laporan ini untuk divisi: {data['dept_name']} periode: {data['period']}.

Gunakan data penggunaan berikut untuk menyusun analisis:
- Chat Sessions: {format_id(data['chat_sessions'])}
- Billing Sessions: {format_id(data['billing_sessions'])}
- Total Input Tokens: {format_id(data['input_tokens'])}
- Total Output Tokens: {format_id(data['output_tokens'])}
- Persentase Over-billing: {data['over_billing_pct']:.1f}% ({data['over_billing_count']} dari {data['chat_sessions']} sesi)
- Penyebab Over-billing:
...
{custom_prompt or "Tidak ada instruksi tambahan."}
